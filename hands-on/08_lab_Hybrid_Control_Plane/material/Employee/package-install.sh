...
# === Defaults ===
DEFAULT_PORT="5555"
INBOUND_PATH_IN_CONTAINER="/opt/webmethods/IntegrationServer/replicate/inbound"
...
function copy_package_into_container() {
  ...
  echo "🔧 Setting ownership and permissions of $PACKAGE_FILE on host..."
  sudo chown 1724:1724 "$PACKAGE_FILE"
  sudo chmod 777 "$PACKAGE_FILE" || {
    echo "❌ ERROR: Failed to change ownership/permissions of package file on host."
    exit 3
  }
